...
import { Card, Cardbg, CardContent, CardImage } from "./ProfileCard";
// import "./Page.css";
...
const companies = [
  ...
  "ASER CENTRE",
  "SUDARSHAN NEWS CHANNEL PVT LTD.",
  "TYRESNMORE",
  "HYPERSPACE CONSULTING",
      ...
      const fetchAlumni = async () => {
        try {
          ...
          const response = await fetch(
            `http://127.0.0.1:5000/alumni?company=${encodeURIComponent(
              selectedCompany
            )}`
          );
          if (!response.ok) {
              ...
              {alumniList.map((alumni, index) => (
                <Card key={index}>
                  ...
                  <CardContent>
                    ...
                    <h2>
                      {alumni.name} <span>({alumni.year_of_placement})</span>
                    </h2>
                    <p>{alumni.position}</p>
